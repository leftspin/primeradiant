defmodule Primeradiant.StorageHarness.DurableSoupDb do
  ...
  def load_soup_ready_facts(db_path, tenant_id) do
    if File.regular?(db_path) do
      ...
      if Enum.all?(required_tables, &table_exists?(db_path, &1)) do
        ...
        db_path
        |> load_soup_ready_projection(tenant_id)
        |> Primeradiant.Soup.ready_facts()
      end
    else
      empty_soup_ready_facts(tenant_id)
    end
  end

  defp load_existing_soup_ready_facts(db_path, tenant_id) do
      ...
      query_table_json(
        ...
        WITH visible_stories AS (
          ...
          WHERE tenant_id = #{quoted_tenant}
            AND COALESCE(json_type(attrs, '$.t1421_quarantine.active_product_truth'), '') != 'false'
        ),
        ranked_cards AS (
          SELECT
            story_id,
    ...
    {failure_streak_count, failure_reasons} =
      load_soup_ready_failure_facts(db_path, quoted_tenant)
  ...
  defp empty_soup_ready_facts(tenant_id) do
    %{
      tenant_id: tenant_id,
    ...
    rows =
      query_table_json(
        ...
        WITH hourly_cards AS (
          SELECT
            row_number() OVER (ORDER BY inserted_at DESC, card_version DESC, rowid ASC) AS row_number,
            COALESCE(
        ...
        failure_streak AS (
          SELECT hourly_cards.row_number, hourly_cards.reason
        ...
        SELECT
          reason,
          (SELECT COUNT(*) FROM failure_streak) AS failure_streak_count
        FROM failure_streak
        GROUP BY reason
        ORDER BY MAX(row_number) DESC;
        """
      )

    failure_streak_count = rows |> List.first(%{}) |> Map.get("failure_streak_count", 0)
    {failure_streak_count, Enum.map(rows, & &1["reason"])}
  end
